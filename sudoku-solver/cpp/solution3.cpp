...
private:
    struct Position {
        ...
        Position(unsigned long long val) {
            x = val & 0xffffffff;
            y = val >> 32;
        }
    };
    ...
    public:
        ...
        void insert(int x, int y) {
            ...
            unsigned long long index = x + (y * 9);
            tmp[index / 8] |= (1 << (index % 8));
        }
        ...
        unsigned long long next() {
            ...
            while (!(tmp[current/8] & (1 << (current % 8)))) {
                current = (current + 1) % 81;
            }
            
            result |= ((unsigned long long) current / 9) << (sizeof(int) * 8); // y
            result |= current % 9; // x

            tmp[current/8] &= ~(1 << (current % 8));
            return result;
        }
        
        void clear() {
            long long* tmp = (long long*) buffer;
            tmp[0] = 0;
            tmp[1] = 0; 
            current = 0;
        }
    };
    NotificationQueue notifications_ = NotificationQueue();
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (count == 1 && !(possibilities_[base_y + j][base_x + i] & SOLVED)) {
                    ...
                    notifications_.insert(base_x + i, base_y + j);
                } else if (count == 0) {
                    cout << "Failed";
                    debug(base_x + i, base_y + j)
                    return false;
    ...
    bool SolveWithBacktracking(int layer = 0) {
        ...
        while (!isDone() && !notifications_.empty()) {
            ...
            while (!notifications_.empty()) {
                Position notification = notifications_.next();
